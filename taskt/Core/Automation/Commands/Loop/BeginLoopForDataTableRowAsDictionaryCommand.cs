...
namespace taskt.Core.Automation.Commands
{
    ...
    [Attributes.ClassAttributes.CommandSettings("Loop For DataTable Row As Dictionary")]
    ...
    public sealed class BeginLoopForDataTableRowAsDictionaryCommand : ADataTableGetFromDataTableCommands, IHaveLoopAdditionalCommands
    {
        //[XmlAttribute]
        ...
        [PropertyValidationRule("Dictionary Variable", PropertyValidationRule.ValidationRuleFlags.None)]
        ...
        public BeginLoopForDataTableRowAsDictionaryCommand()
        {
        }

        public override void RunCommand(Engine.AutomationEngineInstance engine, Script.ScriptAction parentCommand)
        {
            var loopCommand = (BeginLoopForDataTableRowAsDictionaryCommand)parentCommand.ScriptCommand;
            ...
            Action<int> dictionaryVariableAction;
            if (string.IsNullOrEmpty(v_Result))
            {
                dictionaryVariableAction = new Action<int>(row => { }); // nothing
            }
            else
            {
                dictionaryVariableAction = new Action<int>(row =>
                {
                    var cnvDic = new ConvertDataTableRowToDictionaryCommand()
                    {
                        v_DataTable = this.v_DataTable,
                        v_RowIndex = row.ToString(),
                        v_Result = this.v_Result,
                    };
                    cnvDic.RunCommand(engine);
                });
            }
