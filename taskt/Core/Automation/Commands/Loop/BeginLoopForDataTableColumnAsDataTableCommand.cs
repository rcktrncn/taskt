...
namespace taskt.Core.Automation.Commands
{
    ...
    [Attributes.ClassAttributes.CommandSettings("Loop For DataTable Column As DataTable")]
    ...
    public sealed class BeginLoopForDataTableColumnAsDataTableCommand : ADataTableGetFromDataTableCommands, IHaveLoopAdditionalCommands
    {
        //[XmlAttribute]
        ...
        public BeginLoopForDataTableColumnAsDataTableCommand()
        {
        }

        public override void RunCommand(Engine.AutomationEngineInstance engine, Script.ScriptAction parentCommand)
        {
            var loopCommand = (BeginLoopForDataTableColumnAsDataTableCommand)parentCommand.ScriptCommand;
            ...
            {
                dataTableVariableAction = new Action<int>((column) => { });  // nothing
            }
            else
            {
                dataTableVariableAction = new Action<int>((column) =>
                {
                    var cnvTbl = new ConvertDataTableColumnToDataTableCommand()
                    {
            ...
            var loopBodyProcess = new Action<int>((column) =>
            {
                ...
                foreach (var cmd in parentCommand.AdditionalScriptCommands)
                {
                    ...
                    dataTableVariableAction(column);
